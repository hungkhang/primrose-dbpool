...
public class WebConsole { 
    ...
    public static void shutdown() { 
        try { 
            ...
            // This bumps it out of the blocking while loop on accept() 
    ...
    class WebConsoleStarter extends Thread { 
        public void run() { 
            ...
            try { 
                ...
                logger.error("Cannot start WebConsole on port " + port + " : " + 
                    ioe); 
    ...
    class SocketHandler extends Thread { 
        ...
        private void handleRequest() throws IOException { 
            ...
            if ((resource == null) || (resource.length() == 0) || 
                    resource.equals("/") || resource.equals("/index.html")) { 
                ...
                for (Pool pool : loadedPools) { 
                    writeData("<p>"); 
                    writeData("<a href=\"/showPool?poolName=" + 
                        pool.getPoolName() + "\">Show '" + pool.getPoolName() + 
                        "' Pool</a>"); 
                    writeData("</p>"); 
                } 
                ...
                if (pool != null) { 
                    ...
                    writeData( 
                        "<br/><form method=\"get\" action=\"alterPoolProperties\"><table>"); 
 
                    Method[] publicMethods = PoolConfigImpl.class.getMethods(); 
 
                    for (int j = 0; j < publicMethods.length; j++) { 
                        //int numOfArgs = publicMethods[j].getParameterTypes().length; 
                        ...
                        if (methodName.startsWith("get")) { 
                            try { 
                                Object ret = publicMethods[j].invoke(pool, 
                                        new Object[] {  }); 
                    ...
                    writeData( 
                        "<p>If you update the pool settings, the existing pool will be restarted in order for the settings to take effect. Any running connections/SQL will NOT be affected - their jobs will finish normally, and then those connections will be retired.</p>"); 
                } else { 
                    writeData("<p>Sorry, cannot find pool under name '" + 
                        poolName + "'</p>"); 
                } 
                ...
                writeHeaders("text/html", 
                    "/showPool?poolName=" + pool.getPoolName(), 302, 0); 
                ...
                if (pool != null) { 
                    ...
                    if (numberOfWaitingThreads > 0) { 
                        writeData( 
                            "<tr><td><span style='color:red'>Warning ! There are " + 
                            numberOfWaitingThreads + 
                            " threads waiting for a connection.</span> You may want to increase your base pool size.</td></tr>"); 
                    } 
 
                    writeData( 
                        "<tr><td>Total number of connections handed out : " + 
                    ...
                    for (ConnectionHolder ch : connections) { 
                        ...
                        if (ch.status == Pool.CONNECTION_ACTIVE) { 
                            status = "<span style='color:red'>" + 
                                PoolData.getStringStatus(ch.status) + 
                                "</span>"; 
                            inusefor = ((System.currentTimeMillis() - 
                                ch.connOpenedDate) / 1000) + " secs"; 
                            ...
                            if (ch.lastUsedTimestamp > 0L) { 
                                idlefor = ((System.currentTimeMillis() - 
                                    ch.lastUsedTimestamp) / 1000) + " secs"; 
                            } 
                        } 
                    ...
                    writeData("<br/><p>" + 
                        "ID - The PoolConnection class hashCode()<br/>" + 
                        "Status - The Status of the connection<br/>" + 
                        "Opens - How many times this connection has been extracted from the pool<br/>" + 
                        "Closes - How many times this connection has been returned to the pool<br/>" + 
                        "CallableStatement - How many JDBC CallableStatement objects have been created from this connection<br/>" + 
                        "PreparedStatement - How many JDBC PreparedStatement objects have been created from this connection<br/>" + 
                        "Statement - How many JDBC Statement objects have been created from this connection<br/>" + 
                        "Active For - How many seconds this connection has been in use for(if active)<br/>" + 
                        "Idle For - How many seconds this connection has been idle for (if inactive)<br/>" + 
                        "SQL - The current executing SQL on this connection (if any)</p>"); 
                } else { 
                    writeData("<p>Sorry, cannot find pool under name '" + 
                        poolName + "'</p>"); 
                } 
                ...
                Pool pool = findPoolByName(httpRequest.parameters.get( 
                            "PoolName")); 
                Iterator<String> parametersIter = httpRequest.parameters.keySet() 
                                                                        .iterator(); 
                ...
                while (parametersIter.hasNext()) { 
                    ...
                    if (!key.equals("PoolName")) { 
                        try { 
                            logger.info( 
                                "[WebConsole] alterPoolProperties() Setting " + 
                                key + "=" + value); 
                            Util.callClassMethod(Pool.class, pool, "set" + key, 
                                new Object[] { value }); 
                        } catch (GeneralException ge) { 
                            showErrorPage("Error setting property " + key + 
                                " : " + ge); 
        ...
        private void writeHeaders(String contentType, String szExtraData, 
            int iHttpStatusCode, int iContentLength) throws IOException { 
        ...
        private void writeTop(String refreshRate) throws IOException { 
            ...
            if (refreshRate != null) { 
                writeData("<meta http-equiv=\"refresh\" content=\"" + 
                    refreshRate + "\"/>"); 
            } 
        ...
        private void writeCSS() throws IOException { 
            ...
            writeData( 
                "	min-width: 0em;	/* IE doens't understand this property. EMs are used"); 
            writeData( 
            ...
            writeData("{"); 
            writeData( 
                "	border-left-color: #CCCCFF;	/* left column background color */"); 
            writeData( 
                "	border-right-color: #CCCCFF;	/* right column background color */"); 
            writeData( 
